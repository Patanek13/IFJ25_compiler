...
// Max length for mangled names
#define MAX_MANGLED_NAME_LEN 256
 ...
 */
static void init_Scope_Stack(ScopeStack *stack) { stack->topIndex = -1; }
...
static bool is_numeric_ret_func(const char *func_name) {
  return (strcmp(func_name, "Ifj.read_num") == 0 || strcmp(func_name, "Ifj.length") == 0 ||
          strcmp(func_name, "Ifj.floor") == 0) ||
         strcmp(func_name, "Ifj.strcmp") == 0 || strcmp(func_name, "Ifj.ord") == 0;
}
 ...
 */
static bool is_num_type(DataType type) { return type == TYPE_INT || type == TYPE_FLOAT; }
 ...
 * @param error_code Pointer to the error code variable
 */
static void fill_global_table(SymTable *global_table, int *error_code) {
  char *key = NULL;
  SymbolData data;
 ...
 * @param phase Current analysis phase (definition or full analysis)
 */
static void analyze_node(ASTNode *node, AnalysisContext *context, AnalysisPhase phase) {
 ...
 * @param context Pointer to the analysis context
 */
static void analyze_var_decl(ASTNode *node, AnalysisContext *context) {
  if (context->debug) {
  ...
  char mangled_name[MAX_MANGLED_NAME_LEN];
...
static void analyze_block(ASTNode *node, AnalysisContext *context, AnalysisPhase phase) {
  ...
  if (phase == PHASE_ANALYSIS && node->parent->type != NODE_FUNCTION && node->parent->type != NODE_SETTER &&
 ...
 * @param node Pointer to the assign ASTNode.
 * @param context Pointer to the analysis context
 */
static void analyze_assign(ASTNode *node, AnalysisContext *context) {
    ...
    if (setter_data != NULL && setter_data->kind == SYM_SETTER) {
      ...
      if (func_exists(context->global_table, var_name)) {
        *context->error_code = ERR_SEMANTIC_OTHER; // Function name used as variable
        if (context->debug)
          fprintf(stderr, "Semantic Error: Assignment to function name '%s'\n", var_name);
        return;
      }
      ...
      if (getter_data != NULL && getter_data->kind == SYM_GETTER) {
        *context->error_code = ERR_SEMANTIC_OTHER; // Getter name used as variable
        if (context->debug)
          fprintf(stderr, "Semantic Error: Assignment to getter name '%s'\n", var_name);
        return;
      }
  ...
  if (var_symbol->kind == SYM_SETTER) {
    ...
    if (var_symbol->info.setter.param_type != expr_type && var_symbol->info.setter.param_type != TYPE_UNKNOWN &&
...
/**
 * @brief Analyzes the if AST node
 * @param node Pointer to the if ASTNode.
 * @param context Pointer to the analysis context
 * @param phase Current analysis phase (definition or full analysis)
 */
static void analyze_if(ASTNode *node, AnalysisContext *context, AnalysisPhase phase) {
...
/**
 * @brief Analyzes an expression AST node and returns its data type
 * @param node Pointer to the expression ASTNode.
 * @param context Pointer to the analysis context
...
static DataType analyze_expression(ASTNode *node, AnalysisContext *context) {
  if (context->debug) {
    ...
    if (node->type == NODE_BINOP)
      fprintf(stderr, "DEBUG: Binop operator: %s\n", node->value);
    if (node->type == NODE_TYPE_ID)
      fprintf(stderr, "DEBUG: Type ID: %s\n", node->value);
  }
  ...
  case NODE_ID: {
    ...
    // It is a GETTER -> TRANSFORM TO NODE_CALL
    ...
    // Other kinds (FUNC, SETTER) cannot be used as variables
      ...
      *context->error_code = ERR_SEMANTIC_OTHER;
      if (context->debug)
        fprintf(stderr, "Semantic Error: Invalid use of function/setter '%s' as variable\n", var_name);
      return TYPE_UNKNOWN;
    }
    break;
  }
...
/**
 * @brief Analyzes the getter AST node
 * @param node Pointer to the getter ASTNode.
 * @param context Pointer to the analysis context
 * @param phase Current analysis phase (definition or full analysis)
 */
static void analyze_setter(ASTNode *node, AnalysisContext *context, AnalysisPhase phase) {
