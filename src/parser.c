...
#define NUM_BUFFER_SIZE 50 // Maximalni delka retezce pro token hodnotu
#define MAX_HANDLE_SIZE 10 // Maximalni delka zasobniku pro handle
...
char *built_in_string[] = {"read_str",  "read_num", "write", "floor", "str", "length",
                           "substring", "strcmp",   "ord",   "chr",   "read_bool"};
 ...
 * @ param None.
 * @ return Boolean indicating whether the token is a built-in function.
 */
bool is_built_in_func() {
  for (unsigned long idx = 0; idx < sizeof(built_in_string) / sizeof(built_in_string[0]); idx++) {
    if (strcmp(token.value.string, built_in_string[idx]) == 0) {
      return true;
    }
  }
  return false;
...
bool ast_stack_is_empty(const ASTStack *stack) { return stack->topIndex == -1; }
...
static int do_reduction(Stack *tokenStack, ASTStack *astStack, int *error_code) {
  ...
  // Pravidlo 2: E -> ( E )
  else if (handle_len == 3 && handle[2].type == BRACKET_START && handle[1].type == PSEUDO_E &&
           handle[0].type == BRACKET_END) {
    // new_node = NULL; // Uzel uz je na astStack, nic nedelame
    // Debug vypis
  ...
  // Handle (reversed) je [E, op]
    ...
    if (is_unary) {
      ...
      if (op_token.type == OPERATOR)
        op_str = op_token.value.string;
      else if (op_token.type == IS)
        op_str = "is";
      else if (op_token.type == EQUAL_EQUAL)
        op_str = "==";
      else if (op_token.type == NOT_EQUAL)
        op_str = "!=";
      else if (op_token.type == LESS)
        op_str = "<";
      else if (op_token.type == MORE)
        op_str = ">";
      else if (op_token.type == LESS_EQUAL)
        op_str = "<=";
      else if (op_token.type == MORE_EQUAL)
        op_str = ">=";
  ...
  // Pravidlo 6: E -> E ? E : E
  else if (handle_len == 4 && handle[3].type == QUESTION && handle[2].type == PSEUDO_E && handle[1].type == COLON &&
           handle[0].type == PSEUDO_E) {
  ...
  *error_code = ERR_OK;
  ...
  do {
    ...
    while (token.type == NEW_LINE) {
      ...
      if ((top_idx == IDX_MUL) || (top_idx == IDX_LBR) || (top_idx == IDX_QMARK) || (top_idx == IDX_COLON) ||
          (top_idx == IDX_ADD) || (top_idx == IDX_CMP) || (top_idx == IDX_IS) || (top_idx == IDX_EQ) ||
    ...
    if (is_new_token) {
      if ((current_token.type == NEW_LINE) &&
          ((top_terminal.type == OPERATOR) || (top_terminal.type == QUESTION) || (top_terminal.type == COLON))) {
        token = get_token();
        fprintf(out, "---------\ngetting next token\n-----------\n");
        current_token = token;
    ...
    case E: // Equal '=' (pro zavorky)
      is_new_token = true;
      if (rule == E)
        fprintf(out, "DEBUG: Pravidlo EQUAL =\n");
...
ASTNode *cond_loop(int *error_code) {
  *error_code = ERR_OK;
  ...
  if (token.type == IF) {
    // debug vypis
    fprintf(out, "Jsme v cond_loop, nasli sme IF\n");
    print_token(token);
    fprintf(out, "\n");
    ...
    // debug vypis
    fprintf(out, "Jsme v cond_loop, nasli sme WHILE\n");
    print_token(token);
    fprintf(out, "\n");
