...
static KeywordEntry keyword_table[] = {{"class", CLASS},
                                       {"if", IF},
                                       {"else", ELSE},
                                       {"is", IS},
                                       {"null", NULL_KEYWORD},
...
void replace(char ch) { buffer[i - 1] = ch; }
...
Token scan_operator(char op) {
  ...
  case '{':
    return add_token(BLOCK_START);
  case '}':
    return add_token(BLOCK_END);
  case '(':
    return add_token(BRACKET_START);
  case ')':
    return add_token(BRACKET_END);
  case '.':
  ...
  case '*':
    return add_token(MULTIPLY);
  case '/':
    return add_token(DIVIDE);
  case ':':
    return add_token(COLON);
  case '?':
    return add_token(QUESTION);
  default:
    return add_token(ERROR);
  }
}
...
void print_token(Token token) {
  ...
  case FOR:
    ...
    break;
  case NUM_TYPE:
    fprintf(output, "NUM_TYPE");
    break;
  case STR_TYPE:
    fprintf(output, "STR_TYPE");
    break;
  case NULL_TYPE:
    fprintf(output, "NULL_TYPE");
    break;
  case BOOL_TYPE:
    fprintf(output, "BOOL_TYPE");
    break;
  case INTEGER:
    fprintf(output, "INTEGER");
    break;
  case FLOATING:
    fprintf(output, "FLOATING");
    break;
  case STRING:
  ...
  case BOOLEAN:
    ...
    break;
  case BLOCK_START:
    fprintf(output, "BLOCK_START");
    break;
  case BLOCK_END:
    fprintf(output, "BLOCK_END");
    break;
  case BRACKET_START:
    fprintf(output, "BRACKET_START");
    break;
  case BRACKET_END:
    fprintf(output, "BRACKET_END");
    break;
  case COLON:
  ...
  case EQUAL:
    ...
    break;
  case EQUAL_EQUAL:
    fprintf(output, "EQUAL_EQUAL");
    break;
  case LESS:
    fprintf(output, "LESS");
    break;
  case LESS_EQUAL:
    fprintf(output, "LESS_EQUAL");
    break;
  case MORE:
    fprintf(output, "MORE");
    break;
  case MORE_EQUAL:
    fprintf(output, "MORE_EQUAL");
    break;
  case NOT:
    fprintf(output, "NOT");
    break;
  case NOT_EQUAL:
    fprintf(output, "NOT_EQUAL");
    break;
  case AND:
    fprintf(output, "AND");
    break;
