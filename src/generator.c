...
ErrorCode generate_builtIn(ASTNode* node, Frame* gf) {
  ...
  if (strcmp(funId, "Ifj.write") == 0) {
    ...
    fprintf(stdout, "JUMPIFEQ $ifjsub$loopEnd$%i bool@false\n", uniqueId);
    ...
    fprintf(stdout, "$ifjsub$loopEnd$%i\n", uniqueId);
    fprintf(stdout, "PUSHS GF@__$tempRes\n");
  ...
  switch (node->type) {
    ...
    case NODE_UNOP:{
      ...
      if (strcmp(node->value, "-") == 0) {
        ...
        fprintf(stdout, "JUMPIFEQ $unop$not$nil$%i\n", uniqueId);
        fprintf(stdout, "JUMPIFEQ $unop$not$bool$%i\n", uniqueId);
